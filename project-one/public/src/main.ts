...
class ProductSearch {
  ...
  constructor() {
    ...
    this.updateNutriScoreChart();   // Chart initial laden
    this.updateScanHistory();       // Scan-Historie beim Start laden
  }

  private initializeEventListeners(): void {
    // Suchfunktion beim Klick oder Enter
    ...
    // Barcode-Scanner starten/stoppen
  ...
  private initializeSocketListeners(): void {
    // Echtzeit-Aktualisierung bei neuen Scan-Items
  ...
  private async searchProduct(): Promise<void> {
    ...
    this.searchButton && (this.searchButton.textContent = 'Searching...');
  ...
  private mapOpenFoodFactsProductToScanItem(product: any) {
    ...
    return {
      ...
      ecoScoreCategory: (product.ecoscore_grade || product.ecoscore)?.toString().toUpperCase() || 'UNKNOWN',
      nutriScoreCategory: (product.nutriscore_grade || product.nutriscore)?.toString().toUpperCase() || 'UNKNOWN',
  ...
  private async updateNutriScoreChart() {
    // Diagramm mit aktuellen Durchschnittswerten anzeigen
    try {
      ...
      if (this.nutriScoreChart) this.nutriScoreChart.destroy();

      this.nutriScoreChart = new Chart(ctx, {
        type: 'pie',
        data: { labels, datasets: [{ data: counts, backgroundColor: ['#ff6384','#36a2eb','#ffcd56','#4bc0c0','#9966ff'] }] },
  ...
  private async updateScanHistory() {
    // Historie der gescannten Produkte anzeigen
    try {
      // const res = await fetch('/api/scanItem');
      const res = await fetch('/api/scanItem', {
        credentials: "include"
      ...
      container.innerHTML = '';
      noHistory.classList.toggle('d-none', items.length > 0);
